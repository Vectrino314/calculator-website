<template>
  <div>
    ...
    <b-table :data="state.context.savings" :bordered="true" :striped="true">
      ...
      <b-table-column field="actions" label="Actions" centered v-slot="props">
        <button @click="send({ type: 'EDIT_SAVINGS', value: props.row.id })">
          Edit
        </button>
        <button @click="send({ type: 'DELETE_SAVINGS', value: props.row })">
          Delete
        </button>
      </b-table-column>
...
import AddSavingsModal from "./AddSavingsModal.vue";
...
export default {
  ...
  //     this.savings = localStorage.savings;
  //   }
  // },
  // watch: {
  //   savings(newsavings) {
  //     localStorage.savings = newsavings;
  //   }
  // },
  methods: {
      ...
      this.$buefy.modal.open({
        ...
        events: {
          submit: (value) => {
            ...
            this.send({ type: "ADD_SAVINGS", value: value });
