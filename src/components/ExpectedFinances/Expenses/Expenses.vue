<template>
  <div>
    ...
    <b-table :data="state.context.expenses" :bordered="true" :striped="true">
      ...
      <b-table-column field="actions" label="Actions" centered v-slot="props">
        <button @click="send({ type: 'EDIT_EXPENSE', value: props.row.id })">
          Edit
        </button>
        <button @click="send({ type: 'DELETE_EXPENSE', value: props.row })">
          Delete
        </button>
      </b-table-column>
      ...
      this.$buefy.modal.open({
        ...
        events: {
          submit: (value) => {
            ...
            this.send({ type: "ADD_EXPENSE", value: value });
